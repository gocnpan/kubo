...
func LibP2P(bcfg *BuildCfg, cfg *config.Config, userResourceOverrides rcmgr.PartialLimitConfig) fx.Option {
	...
	if enableAutoTLS {
		...
		if !wssWildcardPresent {
			logger.Fatal(fmt.Sprintf("Invalid configuration: AutoTLS.Enabled=true requires a catch-all Addresses.Swarm listener ending with %q to be present, see https://github.com/gocnpan/kubo/blob/master/docs/config.md#autotls", wssWildcard))
		}
	}
